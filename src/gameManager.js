...
const UsersCreate = (request, response, name) => {
  ...
  let possesiveName = '';
  name = name.trim();
  if (name.toLowerCase()[name.length - 1] !== 's') { // The last character isn't an S
...
const GameAvailableGames = (request, response) => {
  ...
  for (let i = 0; i < games.length; i++) {
    if (games[i].players.length === 1 && games[i].deleted_at == null) { // The game only has 1 player and isn't deleted
      // Add the game
...
const GameUpdate = (gameId) => {
  // If this function is called, its assumed that the gameId was already checked for it to exist and the game is not deleted
  ...
  if (games[gameId].players.length != 2) { // It does not yet have both players
    games[gameId].state = "Waiting for game to start";
    return;
  }
  ...
  if (!games[gameId].roundEnd) { // It is in a round right now
    ...
    switch (readyPlayers) {
      ...
      case 1: // 1 player is ready and 1 isn't
        games[gameId].state = `Waiting for ${games[gameId].players[notReadyPlayerIndex].user.name}`;
        break;
